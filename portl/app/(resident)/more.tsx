...
const menu: { section: string; items: { label: string; icon: keyof typeof Feather.glyphMap; route: string }[] }[] = [
  {
    ...
    items: [
      ...
      { label: "Community polls", icon: "bar-chart-2", route: "/polls" },
      { label: "Staff & service directory", icon: "users", route: "/staff-directory" },
    ],
  },
  {
    section: "Payments & history",
    items: [
      { label: "Maintenance dues", icon: "credit-card", route: "/payments" },
      { label: "Visitor history", icon: "clock", route: "/(resident)/visitors" },
    ],
  },
  {
    section: "Account",
    items: [
      ...
      { label: "Notification settings", icon: "sliders", route: "/profile" },
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        <View className="px-5 pt-3 pb-5 flex-row items-center gap-4">
          ...
          <View>
            ...
            <Text className="text-ink400 text-sm mt-0.5">
              {user?.towerName} · {user?.flatLabel} · +91 {user?.phone}
            </Text>
          </View>
        </View>
              ...
              {section.items.map((item, i) => (
                <Pressable
                  ...
                  className={`flex-row items-center px-4 py-3.5 ${
                    i < section.items.length - 1 ? "border-b border-ink50" : ""
                  }`}
                >
